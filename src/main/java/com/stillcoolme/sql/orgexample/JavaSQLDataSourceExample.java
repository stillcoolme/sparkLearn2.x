...
//    runJdbcDatasetExample(spark);
  ...
  private static void runBasicDataSourceExample(SparkSession spark) {
    ...
    peopleDF.select("name", "age").write().format("parquet").save("namesAndAges.parquet");  //any data source type can be converted into other types
     ...
     * Persistent tables will still exist even after your Spark program has restarted, as long as you maintain your connection to the same metastore.
        不同于 createOrReplaceTempView方法， saveAsTable方法会 实现DataFrame的内容 并 给在Hive metastore创建 data的指针。
        即使spark重启，持久化表也将一直存在，直到你maintain你的连接到相同的metastore中？？
  ...
  private static void runParquetSchemaMergingExample(SparkSession spark) {
    ...
    // Read the partitioned table ， 先打开 合并的配置  mergeSchema
  ...
  private static void runJsonDatasetExample(SparkSession spark) {
    ...
    Dataset<Row> anotherPeople = spark.read().json((Seq<String>) anotherPeopleDataset);
    anotherPeople.show();
    // +---------------+----+
    // |        address|name|
    // +---------------+----+
